...
It seems that this may be what prevents it from matching the relevant packages, leading to the error we get: it's looking for an ID that's new `cargo metadata` format, but IFD-generated `crate-hashes.json` is old metadata format.
